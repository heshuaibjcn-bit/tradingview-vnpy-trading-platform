...
try:
    result = subprocess.run(
        ["bash", "-lc", "ps aux | grep -i 'python.*start_vnpy' | grep -v grep"],
        capture_output=True,
        text=True,
        timeout=5
    )
...
try:
    ...
    # 检查事件引擎是否已启动
    assert main_engine.event_engine.is_active(), "事件引擎未激活"
    print("✅ 事件引擎已激活")
...
try:
    ...
    assert main_engine3.event_engine.is_active(), "事件引擎未激活"
    print("✅ 事件引擎运行正常")
